...
function init(user,pass,host) {
    ...
    verto = new Verto({
        login: user+"@"+host,
        passwd: pass,
        socketUrl: location.protocol.indexOf('https:')==0?"wss://"+host+":8082":"ws://"+host+":8081",
        //tag: setRemoteStrean,
        tag:"webcam",
        ...
        { (this.state.status == 'ready' || this.state.status =='closed' || this.state.status =='main')?
          (<View>
            ...
            <TouchableHighlight
              ...
              <Button title='Logout' style={styles.registerbutton} onPress={(e) => this.logout(e)}>退出</Button>
          </View>) : null
        }
          { (this.state.status == 'connected' || this.state.status == 'completed' || this.state.status== 'incall')?
              (<View>
                      <Button title='Hangup' style={styles.registerbutton} onPress={(e) => this.hangup(e)}>挂断</Button>
                  </View>
              ) : null
          }
