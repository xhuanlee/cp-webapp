...
class VertoDialplanLink extends Component {
  constructor(props) {
    super(props);
    this.state = {
      timeStr: '0:00',
  ...
  componentWillReceiveProps(nextProps) {
    const { dialplan } = this.props.match.params;
    if (dialplan.indexOf('@') > 0 && dialplan.indexOf('@gfax.cn') < 0) {
      callDist = dialplan.replace('@', '#');
    } else {
      callDist = dialplan;
    }
    this.setState({ callDist });
    const { ipLoading } = nextProps.client;
    if (!ipLoading) {
