...
class VertoDialplanGroupLink extends Component {
  constructor(props) {
    super(props);
    this.state = {
      timeStr: '0:00',
  ...
  componentWillMount() {
    ...
    this.props.dispatch({ type: 'client/fetchGroupByUrl', payload: { cpUrl: group } });
  }
  componentDidMount() {
    // 初始化数据
    const { group } = this.props.match.params;
  ...
  componentWillReceiveProps(nextProps) {
    const { extension } = this.props.match.params;
    const { group, ipLoading, groupLoading } = nextProps.client;
    if (group && !ipLoading && !groupLoading) {
      ...
      if (extension) {
        this.setState({ callDist: `${sammelnummer}${extension}` });
        callDist = `${sammelnummer}${extension}`;
      } else {
        this.setState({ callDist: sammelnummer });
        callDist = sammelnummer;
      }
      console.log('数据初始化完成');
      // 只进行一次呼叫
  ...
  render() {
    ...
    return (
      <div style={pageStyle}>
        ...
        <div style={headStyle}>
          <div>
            <h1 style={{ color: '#fff', border: 'none' }}>{this.state.callDist}</h1>
